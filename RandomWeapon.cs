...
public class RandomWeapon : Weapon
{
    public override void Fire(Vector3 start, Vector3 dir)
    {
        ...
        bullet.rotation = Quaternion.FromToRotation(Vector3.up, randomDir); // 랜덤 방향을 향하도록 회전 설정
        bullet.GetComponent<Bullet>().Init(damage, count, randomDir, Life_time); // 초기화 시 랜덤 방향으로 발사
